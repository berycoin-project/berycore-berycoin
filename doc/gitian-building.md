...
*Setup instructions for a Gitian build of Berycoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Berycoin
...
- [Building Berycoin Core](#building-berycoin-core)
...
Clone the git repositories for berycoin and Gitian.
...
in the berycoin repository under 'Fetch and create inputs' to install sources which require
...
To build Berycoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the berycoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/berycoin/.git/
...
For example:
```bash
URL=https://github.com/thrasher-/berycoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit berycoin=${COMMIT} --url berycoin=${URL} ../berycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit berycoin=${COMMIT} --url berycoin=${URL} ../berycoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit berycoin=${COMMIT} --url berycoin=${URL} ../berycoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the berycoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../berycoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/berycoin
SIGPATH=/some/root/path/berycoin-detached-sigs

./bin/gbuild --url berycoin=${BTCPATH},signature=${SIGPATH} ../berycoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.BERY` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/berycoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/berycoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/berycoin-osx-build.assert
```
...
[gitian.sigs.BERY](https://github.com/berycoin-project/gitian.sigs.BERY/) repository, or if that's not possible create a pull
